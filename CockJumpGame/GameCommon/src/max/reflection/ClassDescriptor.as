...
package max.reflection
{
	...
	public class ClassDescriptor
	{
		...
		/**
		 * All accessable variables and accessors (readwrite)
		 */		
		public const accessables:Array = [];
			...
			for each(var itemXml:XML in xmlList)
			{
				switch (kind)
				{
					case MemberKind.ACCESSOR:
						var accessor:Accessor = new Accessor(itemXml);
						accessors.push(accessor);
						if (!accessor.readonly)
							accessables.push(accessor);
						break;
					case MemberKind.METHOD:
						methods.push(new Method(itemXml));
						break;
					case MemberKind.VARIABLE:
						var variable:Variable = new Variable(itemXml);
						variables.push(variable);
						accessables.push(variable);
						break;
					default:
